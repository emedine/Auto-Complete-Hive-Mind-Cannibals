...
public class Antenna {
  ...
  public void move(float theX, float theY, float degree) {
    ...
    tv += 0.5 * (pApp.random(300) - pApp.random(300)) / 10.0;
...
private void display() {
    ...
    for (int i = 0; i < numNodes-1; i++) {
    	...
    	rnd1 = rnd1 * 255;
    	pApp.fill(theFillColor, rnd1);
    	pApp.stroke(theFillColor, rnd1);
    	 // this.lineStyle((this.numNodes/(i-1))*1.5, 0xFFFFFF, 100);  // with head
        // this.pApp.lineStyle((this.numNodes-i), newFillColor, 100);  // with no head

    	pApp.strokeWeight(pApp.sq(numNodes - i) / 3);
